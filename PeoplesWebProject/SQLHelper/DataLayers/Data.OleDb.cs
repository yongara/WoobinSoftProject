...
using UCGuideSQLHelper.Base;

namespace UCGuideSQLHelper.OLEDB
{
    public class OleDbAccess : DataAccess
    {
        ...
        {
            return base._getCommand<OleDbCommand>(ref sqlCommand, cmdType);
        }
        ...
        public override DataTable GetData(string sql)
        {
            ...
            try
            {
                DbDataReader reader = base._getReader<OleDbCommand>(ref sql);
                dt.Load(reader);
                reader.Dispose();
            }
            catch (Exception ex)
        ...
        public override int ExecuteNonQuery(string sqlCommand, CommandType cmdType)
        {
            ...
            try
            {
                OleDbCommand cmd = (OleDbCommand)getCommand(ref sqlCommand, cmdType);
        ...
        public override int ExecuteNonQuery(string sql, CommandType cmdType, ParameterEngine param)
        {
            ...
            try
            {
                OleDbCommand cmd = (OleDbCommand)this.GenerateCommand(sql, cmdType, param);
