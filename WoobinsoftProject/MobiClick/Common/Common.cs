    ...
    class Common
    {
        ...
        public static bool IsUserLoginCheck(string userid, string password)
        {
            ...
            try
            {
                ...
                string procedureName = "SP_USER_LOGIN_CHECK";
                DbCommand command = da.GenerateCommand(procedureName, CommandType.StoredProcedure, param);
                string value = da.GetSingleValue(command).ToString();
                if (string.IsNullOrEmpty(value) == true || value == "N")
                {
        ...
        /// <returns></returns>
        public static string InsertUserInfo(string userid, string password, string macAddress, string authCode)
        {
            string result = "001";
            DataAccess da = new SQLServerDB();
            if (!da.IsConnected)
            ...
            try
            {
                ...
                string procedureName = "SP_USERINFO_INSERT";
                DbCommand command = da.GenerateCommand(procedureName, CommandType.StoredProcedure, param);
                result = da.GetSingleValue(command).ToString();
            }
            catch (Exception ex)
            {
                WriteTextLog("InsertUserInfo", ex.Message.ToString());
                result = "005";
            }
            finally
            {
        ...
        #region InsertKeyWordInfo - KeyWord 정보를 추가합니다.
        /// <summary>
        /// KeyWord 정보를 추가합니다.
        /// </summary>
        /// <param name="userid"></param>
        ...
        #region DeleteKeyWordInfo - KeyWord 정보를 삭제합니다.
        /// <summary>
        /// KeyWord 정보를 삭제합니다.
        /// </summary>
        /// <param name="userid"></param>
        ...
        /// <param name="authCode"></param>
        /// <returns></returns>
        public static int DeleteKeyWordInfo(string keycode, string keyword, string url)
        {
            int result = 0;
            DataAccess da = new SQLServerDB();
            if (!da.IsConnected)
            ...
            try
            {
                ...
                string procedureName = "SP_KEYWORDINFO_DELETE";
                DbCommand command = da.GenerateCommand(procedureName, CommandType.StoredProcedure, param);
                result = da.ExecuteNonQuery(command);
            }
            catch (Exception ex)
            {
                WriteTextLog("DeleteKeyWordInfo", ex.Message.ToString());
            }
            finally
            {
                DisConnect(ref da);
        ...
        #region KeyWordRankUpdate - 키워드 순위를 업데이트 합니다.
        /// <summary>
        ///  키워드 순위를 업데이트 합니다.
        /// </summary>
